...
def create_app(test_config=None):
  ...
  '''
    ...
    '''
        try:
            # get the query of question with target question's id
            selection = Question.query.filter_by(id = id).one_or_none()

            # if not found, then arise an error
            if selection == None:
                abort(404)
  ...
  '''
    ...
    def play_quiz():
        try:
            ...
            previous_quiz = body['previous_questions']
            questions = Question.query.filter_by(category=quiz_category['id']).filter(Question.id.notin_(previous_quiz)).all()

            # just get a random question from remaining questions
            q = questions[random.randrange(0, len(questions), 1)]
            previous_quiz.append(q)
